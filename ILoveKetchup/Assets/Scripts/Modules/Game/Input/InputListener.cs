...
#pragma warning restore CS0649
            ...
            if (mouse != null)
            {
                if (Mouse.current.leftButton.wasPressedThisFrame)
                {
                    ...
                    if (!needToCancelDrag)
                    {
                        ...
                        inputActions.OnSelectDown?.Invoke(mouse.position.ReadValue());
                    }
                }
                ...
                if (Mouse.current.leftButton.wasReleasedThisFrame)
                {
                    ...
                    if (CheckDragEnd())
                    {
                        Log("[InputListener] [Mouse] Drag ended by check drag end.");
                    }
                }

                if (!needToCancelDrag && isMousePressing)
                {
                    MousePosition(mouse);
                }
            }
            ...
            if (touch != null)
            {
                if (touch.press.wasPressedThisFrame)
                {
                    if (nextTouchTime < time)
                    {
                        ...
                        {
                            inputActions.OnSelect?.Invoke();
                            inputActions.OnSelectDown?.Invoke(touch.position.ReadValue());
                        }
                    }
                }
        ...
        private void TouchPosition(Touchscreen touch)
        {
            ...
            else
            {
                if (touch.press.wasReleasedThisFrame)
                {
                    if (CheckDragEnd())
                    {
                        Log("[InputListener] [Touch] Drag ended by check drag end.");
                    }
                }
            }
        }
        ...
        private void CheckDragBegin(Vector2 pos)
        {
            ...
            if (Vector2.Distance(pressedPosition, pos) >= minDragMagnitude)
            {
                ...
                Log("[InputListener] Dragging started.");
            }
        }
        ...
        private bool CheckDragEnd()
        {
            if (isDragging)
            {
                ...
                Log("[InputListener] Dragging end.");
